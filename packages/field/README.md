...
<template>
  ...
  <!-- 通过导出的形式直接使用对应的字段组件 -->
  <ProFieldUploadAvatar mode="read"/>
  <!-- 通过对象取值的方式指定渲染的组件 -->
  <ProField.Radio
...
<script setup lang="ts">
  import { ProField, ProFieldUploadAvatar } from "@element-plus-ui/pro-field";
...
- [ProFieldSelect](#ProFieldSelect) 支持 **request** 和 **valueEnum** 两种方式来生成子项，用于从两项以上中选择一项
...
- [ProFieldCascader](#ProFieldCascader) 当一个数据集合有清晰的层级结构时，可通过级联选择器逐级查看并选择，支持 **request** 和 **valueEnum** 两种方式来生成层级结构
- [ProFieldTimeSelect](#ProFieldTimeSelect) 用于选择或输入日期
- [ProFieldTreeSelect](#ProFieldTreeSelect) 支持 **request** 和 **valueEnum** 两种方式来生成子项，用于从两项以上中选择一项
...
属性 `marker`、`request`、`valueEnum`、`separator`、`emptyText`、`placeholder`、`mappingEnumValue` 只在当前 **type** 指定的组件，包含此属性时生效.

| 属性 | 描述          | 类型                                 | 默认值 |
| ---- | ------------- | ------------------------------------ | ------ |
| v-model  | 绑定的值. | `string` / `number` / `boolean` / `Array<string \| number \| boolean>` | -      |
| mode  | 展示模式. | `read` / `edit` | edit      |
| type  | ProField 组件当前渲染的字段类型. | [Type](#Type) | text      |
...
> 如有需要，您可前往 [ElementPlus](https://element-plus.org/zh-CN/) 官方文档查阅对应的组件 **API**

```vue
<template>
  <ProField.Radio v-model="value" :request="getRadioOptions"/>
</template>
...
> 在 **ValueEnum** 配置中，当键值为对象时，除了包含 **status**、**color** 等属性之外、同时也继承了当前渲染的组件自身的 **Props**，如以上设置了 **disabled** 。如有需要，您可查阅 [ElementPlus](https://element-plus.org/zh-CN/) 官方文档.
...
| emptyText  | **mode=read** 时，值为空时的占位文本. | `string` | -      |
| fieldProps  | Radio 组件 Props 配置 | [FieldProps](https://element-plus.org/zh-CN/component/radio.html) | {}      |
...
| mappingEnumValue  | 枚举配置未明确指定 **value** 时，需要映射的目标对象. | `lable` / `index` | lable      |
| emptyText  | **mode=read** 时，值为空时的占位文本. | `string` | -      |
| fieldProps  | RadioButton 组件 Props 配置 | [FieldProps](https://element-plus.org/zh-CN/component/radio.html) | {}      |
...
| emptyText  | **mode=read** 时，值为空时的占位文本. | `string` | -      |
| fieldProps  | RadioButton 组件 Props 配置 | [FieldProps](https://element-plus.org/zh-CN/component/checkbox.html) | {}      |
...
| separator  | 阅读模式下，显示的多个值之间的分隔符. | `string` | -      |
| emptyText  | **mode=read** 时，值为空时的占位文本. | `string` | -      |
| fieldProps  | CheckboxButton 组件 Props 配置 | [FieldProps](https://element-plus.org/zh-CN/component/checkbox.html) | {}      |
...
| mode  | 展示模式. | `read` / `edit` | edit      |
| emptyText  | **mode=read** 时，值为空时的占位文本. | `string` | -      |
| fieldProps  | Color 组件 Props 配置 | [FieldProps](https://element-plus.org/zh-CN/component/color-picker.html) | {}      |
...
| 属性 | 描述          | 类型                                 | 默认值 |
| ---- | ------------- | ------------------------------------ | ------ |
| v-model  | 绑定的值. | `string` / `string[]` | -      |
| mode  | 展示模式. | `read` / `edit` | edit      |
| fieldProps  | UploadImage 组件 Props 配置 | [FieldProps](#) | {}      |

> **fieldProps** 合并了 [ElUpload](https://element-plus.org/zh-CN/component/upload.html) 和 [ElImage](https://element-plus.org/zh-CN/component/image.html) 组件 **Props** 属性，可统一在此进行配置
...
> **fieldProps** 合并了 [ElUpload](https://element-plus.org/zh-CN/component/upload.html) 和 [ElAvatar](https://element-plus.org/zh-CN/component/avatar.html) 组件 **Props** 属性，可统一在此进行配置
...
| separator  | 阅读模式下，显示的多个值之间的分隔符. | `string` | -      |
| emptyText  | **mode=read** 时，值为空时的占位文本. | `string` | -      |
| fieldProps  | TreeSelect 组件 Props 配置 | [FieldProps](https://element-plus.org/zh-CN/component/select-v2.html) | {}      |
| placeholder  | 输入框占位文本，透传到 **FieldProps** 中. | `string` | -      |
