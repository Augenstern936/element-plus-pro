          ...
          return (
                  ...
                  return titleText ? (
                    <>
                      {mark === true || shape === "bar" || !shape ? (
                        <span class="title-mark-shape-bar" style={{ "--mark-color": color, ...textStyle }}>
                          {titleText}
                        </span>
                      ) : (
                        <ul>
            ...
            >
              {descItems?.map((item, index) => {
                ...
                const slotContent = dataField && ctx.slots[dataField] ? ctx.slots[dataField]?.(data) : render?.(data ?? {});
                return (
                  <>
                    {ctx.slots.default ? (
                      <ElDescriptionsItem style={props.bodyStyle ?? {}}>{ctx.slots.default()}</ElDescriptionsItem>
                    ) : (
                      <ElDescriptionsItem
                        {...item}
                        key={index}
